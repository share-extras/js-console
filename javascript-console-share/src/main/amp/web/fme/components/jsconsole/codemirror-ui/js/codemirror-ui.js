...
function CodeMirrorUI(place, options, mirrorOptions) {
  return this.initialize(place, options, mirrorOptions);
}
...
CodeMirrorUI.prototype = {
  ...
  initialize: function(textarea, options, mirrorOptions) {
    var defaultOptions = {
      ...
      imagePath: 'javascript-console/images',
      path: 'js',
    ...
    //console.log(mir);
    this.mirror = mir;
    mir.on("change",function(cm, objChanged){
        ui.editorChanged(cm, objChanged, ui);
    });
    this.initButtons();
  ...
  /**
   * get back the mirror instance itself.
   */
  ...
  editorChanged: function(cm,changeObj, ui) {
    if(!ui.mirror) {
      return
    }
    var his = ui.mirror.historySize();
    if (his['undo'] > 0) {
      ui.removeClass(ui.undoButton, 'inactive');
    } else {
      ui.addClass(ui.undoButton, 'inactive');
    }
    if (his['redo'] > 0) {
      ui.removeClass(ui.redoButton, 'inactive');
    } else {
      ui.addClass(ui.redoButton, 'inactive');
    }
  ...
  jump: function() {
      ...
      {
          title: Alfresco.util.message("jsconsole.editor.jump.title"),
          input: "text",
          callback: {
              fn: function(line){
